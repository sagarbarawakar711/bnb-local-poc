locals {
  ...
  environment_data = {
    ...
    NEXTAUTH_URL            = var.common_tags.Environment == "nonprod" ? "https://dev-admin.sep.nonprod.mheducation.com" : "https://admin.sep.mheducation.com"
  ...
  secrets_data = {
    ...
    OKTA_CLIENT_ID     = "arn:aws:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:/sep/${var.common_tags.Environment}/task_def-${local.secret_manager_arn_id}:ADMIN_OKTA_CLIENT_ID::"
    OKTA_CLIENT_SECRET = "arn:aws:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:/sep/${var.common_tags.Environment}/task_def-${local.secret_manager_arn_id}:ADMIN_OKTA_CLIENT_SECRET::"
  }
...
#### DD Integration ######
module "backend_app_dd_container" {
...
resource "aws_ecs_task_definition" "task_definition" {
  ...
  container_definitions = jsonencode([
    module.backend_app_dd_container.fluentbit_container_definition,
    {
      ...
      "logConfiguration" : module.backend_app_dd_container.service_container_definition_log_configuration,
      "dependsOn" : module.backend_app_dd_container.service_container_definition_depends_on,
