...
echo "Current Workspace: $CUR_WORKSPACE"
echo "Target Workspace: $TF_WORKSPACE"

if [ "$CUR_WORKSPACE" != "$TF_WORKSPACE" ]; then
    if terraform workspace list | grep -q "$TF_WORKSPACE"; then
        echo "Selecting existing workspace: $TF_WORKSPACE"
        terraform workspace select "$TF_WORKSPACE" || { echo "Failed to select workspace $TF_WORKSPACE"; exit 1; }
    else
        echo "Creating new workspace: $TF_WORKSPACE"
        terraform workspace new "$TF_WORKSPACE" || { echo "Failed to create workspace $TF_WORKSPACE"; exit 1; }
    fi
fi
...
#terraform import 'aws_s3_bucket.sep_cf_bucket' aid-sep-dev-cf
##
## Plan
##
echo "Intermediate time: $(date)"
