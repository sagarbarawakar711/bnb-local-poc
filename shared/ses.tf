...
locals {
  ...
  # ses_sending_email = [
  #   "tom.schmerbach@mheducation.com",
  #   "udit.meghraj@mheducation.com"
  # ]
}
...
resource "aws_ses_domain_mail_from" "sep" {
...
resource "aws_ses_identity_notification_topic" "sep" {
...
#   for_each = toset(local.ses_sending_email)
#   email    = each.value
# }

resource "aws_ses_identity_policy" "sep" {
  identity = aws_ses_domain_identity.mhe_default.domain
  name     = "sep"
  policy = jsonencode({
    "Version" : "2012-10-17",
